        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(map.size()<myAdapter.getCount()){
                    Toast.makeText(context,"请先输入盘点数量",Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                String url = UrlHelper.URL_HALF_PRODUCT_CHECK_COMMIT.replace("{stockId}", String.valueOf(id)).replace("{spaceStockIds}", spaceStockIds1).replace("{spaceIds}", spaceIds1).replace("{beforeStocks}", beforeStocks1).replace("{afterStocks}", afterStocks1);
...
//                        得到列表的数据源
                        ...
                        myAdapter = new SpaceInfoAdapter(context, list);
                        ...
                        tvStockNum.setText(String.valueOf(halfStock.getStock()));
                    }
                }, new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError volleyError) {
