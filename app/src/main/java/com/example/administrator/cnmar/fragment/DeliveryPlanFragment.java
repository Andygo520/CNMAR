...
public class DeliveryPlanFragment extends Fragment {
    ...
    private String strUrl = UniversalHelper.getTokenUrl(UrlHelper.URL_DELIVERY_PLAN.replace("{page}", String.valueOf(page)));

    public DeliveryPlanFragment() {
...
//                            getPlanListFromNet(url);
        ...
        etSearchInput.setHint("交付计划编号查询");
...
//                            myAdapter.notifyDataSetChanged();
    ...
    class BillAdapter extends BaseAdapter {
        ...
        private List<CustomDeliverPlan> list = null;

        public BillAdapter(List<CustomDeliverPlan> list, Context context) {
            this.list = list;
...
//           没有成品出库单的，显示空字符
            if (list.get(position).getProductOutOrder() == null)
                holder.tvProductOutOrder.setText("");
            else
