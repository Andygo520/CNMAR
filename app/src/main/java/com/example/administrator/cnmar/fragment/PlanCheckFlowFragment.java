                ...
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_UP) {
                    ...
                    if (input.equals("")) {
                        ...
                        String urlString = UrlHelper.URL_SEARCH_CHECK_FLOW
                                .replace("{userId}", SPHelper.getInt(getActivity(), "userId") + "")
                                .replace("{type}", "1").replace("{code}", input);
...
//                偶数行背景设为灰色
            ...
            holder.tvProcess.setText(list.get(position).getPlan().getProcessProduct() == null ? "" : list.get(position).getPlan().getProcessProduct().getName());
