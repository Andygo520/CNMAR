...
public class HalfProductOutOrderFragment extends Fragment {
    ...
    private List<HalfOutOrder> data = new ArrayList<>();
    private String url = UniversalHelper.getTokenUrl(UrlHelper.URL_HALF_PRODUCT_OUT_ORDER.replace("{page}", String.valueOf(page)));

    public HalfProductOutOrderFragment() {
...
//      默认情况下不激活setOnItemSelectedListener方法，只有选择的时候才调用该方法
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String urlString = UrlHelper.URL_SEARCH_HALF_PRODUCT_OUT_ORDER.replace("{query.code}", "").replace("{query.status}", String.valueOf(map.get(status[position])));
...
//                        Log.d("GGGG",json);
                        ...
                        List<HalfOutOrder> list = JSON.parseArray(response.getData().toString(), HalfOutOrder.class);
...
//                            myAdapter.notifyDataSetChanged();
    ...
    class BillAdapter extends BaseAdapter {
        ...
        private List<HalfOutOrder> list = null;

        public BillAdapter(List<HalfOutOrder> list, Context context) {
            this.list = list;
...
//            取出出库时间作为出库批次号
            ...
            holder.tvReceive.setText(list.get(position).getReceive()==null?"":list.get(position).getReceive().getCode());
