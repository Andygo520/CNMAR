 ...
 */
public class ValidBinFragment extends Fragment {
...
//                            当page等于总页数的时候，提示“加载完成”，不能继续上拉加载更多
                        if (page == total) {
                            String url = UniversalHelper.getTokenUrl(UrlHelper.URL_BIN.replace("{type}", BoxTypeVo.success.getKey() + "").replace("{page}", String.valueOf(page)));
                            Log.d("urlfinish", url);
                            getDataFromNet(url);
